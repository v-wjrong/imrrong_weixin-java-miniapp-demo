...
该流程图展示了微信小程序Demo项目的完整目录结构，从根目录binarywang开始，逐级展开到最底层的控制器、配置和工具类文件。核心模块miniapp包含5个子模块：utils工具包存放JSON处理类，error处理异常页面，config管理配置参数，controller包含三个微信功能控制器，同时直接包含主应用类。结构清晰体现了Spring Boot应用的典型分层架构。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [demo](demo/_module.md) | package | JsonUtils工具类处理JSON转换，配置忽略null值并格式化输出。错误处理模块统一管理404/500错误跳转。微信小程序配置模块整合基础属性和消息处理。控制器模块处理媒体文件、用户会话和微信交互。应用入口类WxMaDemoApplication启动Spring Boot应用。 |
